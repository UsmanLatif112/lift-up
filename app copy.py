            ...
            if driver is not None and gl is not None and profile_id is not None:
                try:
                    ...
                    try:
                        ...
                        csvv.make_csv('fragrancedirect.csv', ["No google acceptance modal found", current_time])
                            ...
                            if result_mine:
                                for result in result_mine:
                                    try:
                                        ...
                                        time.sleep(1)
                                        current_time = datetime.datetime.now()
                                        csvv.make_csv('fragrancedirect.csv', [f"title({title}) found in this iteration,{result.text}", current_time])
                                        csvv.make_csv('fragrancedirect.csv', ["\n",])
                                ...
                                current_time = datetime.datetime.now()
                                csvv.make_csv('fragrancedirect.csv', [f"title({title}) Not found in this iteration", current_time])
                                csvv.make_csv('fragrancedirect.csv', ["\n",])
                        except Exception as e:
                            print(e)
                            current_time = datetime.datetime.now()
